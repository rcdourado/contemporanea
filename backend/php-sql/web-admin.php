...
/** admin **/
$config["email"]			= "admin@contemporanea" ; 		//email for login
$config["password"]			= "09117b77238d2bde3626f11cd1a58e88910938c4" ; 		// sha1(password)
$config["utf8"]			= true; 		
...
/** LANGGUAGE:START **/
define("LANG_ARQUITETO","Arquitetos");
define("LANG_ID","Ids");
define("LANG_NOME","Nomes");
define("LANG_FOTO","Fotos");
define("LANG_CONTEUDO","Conteudos");
/** default constant **/
...
if($_SESSION["IS_ADMIN"]==true){
	...
	$tags_html .= "</ul>" ;
	$tags_html .= "<h1 class=\"text-muted\">Contemporanea</h1>" ;
	$tags_html .= "<p>Aplicativo do Projeto contempornea.</p>" ;
	$tags_html .= "</div>" ;
	$tags_html .= "</div>" ;
	$tags_html .= "<div class=\"container-fluid\">" ;
	...
	if($_GET["table"]=="arquitetos"){
		$tags_html .= "<li class=\"active\"><a href='?table=arquitetos'>". @LANG_ARQUITETO."</a></li>" ;
	}else{
		$tags_html .= "<li><a href='?table=arquitetos'>". @LANG_ARQUITETO."</a></li>" ;
	}
	...
	switch($_GET["table"]){
		case "home":
					$tags_html .= "<h4 class=\"page-title\">Contemporanea</h4>" ;
					$tags_html .= "<div class=\"row\">" ;
					$tags_html .= "<div class=\"col-md-4\">" ;
					$tags_html .= "<div class=\"panel panel-primary\" >" ;
					$tags_html .= "<div class=\"panel-heading\">" ;
					$tags_html .= "<h4  class=\"panel-title\"><span class=\"glyphicon glyphicon-list-alt\"></span> ". @LANG_ARQUITETO ."</h4>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-body\" style=\"min-height: 150px;\">" ;
					$tags_html .= "<ul class=\"list\">" ;
					/** fetch data from arquiteto **/
					$sql_query = "SELECT * FROM `arquiteto` ORDER BY `id`  DESC LIMIT 0 , 5" ;
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							$tags_html .= "<li>" . htmlentities(stripslashes($data["nome"])) . "</li>" ;
						}
					}
					$tags_html .= "</ul>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-footer text-right\"><a href=\"?table=arquitetos\" class=\"btn btn-sm btn-primary\">".LANG_MORE."</a></div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "</div>" ;
		break;
		// TODO: Arquitetos
		case "arquitetos":
					$tags_html .= "<h4>". @LANG_ARQUITETO ."</h4>" ;
				switch($_GET["action"]){
					// TODO: ---- listing
					case "list":
					$tags_html .= "<ul class=\"nav nav-tabs\">" ;
					$tags_html .= "<li class=\"active\"><a href=\"?table=arquitetos\">".@LANG_LIST."</a></li>" ;
					$tags_html .= "<li><a href=\"?table=arquitetos&action=add\">".@LANG_ADD."</a></li>" ;
					$tags_html .= "</ul>" ;
					$tags_html .= "<br/>" ;
					...
					$table_html .= "<thead>" ;
					$table_html .= "<tr>" ;
					$table_html .= "<th>".@LANG_NOME."</th>" ;
					$table_html .= "<th>".@LANG_FOTO."</th>" ;
					$table_html .= "<th>".@LANG_CONTEUDO."</th>" ;
					$table_html .= "<th style=\"width:100px;\">Action</th>" ;
					$table_html .= "</tr>" ;
					$table_html .= "</thead>" ;
					$table_html .= "<tbody>" ;
					/** fetch data from mysql **/
					$sql_query = "SELECT * FROM `arquiteto`" ;
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							$table_html .= "<tr>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["nome"])) . "</td>" ;
							$table_html .= "<td><img src=\"" . $data["foto"] . "\" alt=\"#\" width=\"64\" height=\"64\" /></td>" ;
							$content_html = substr( strip_tags($data["conteudo"]),0,50);
							$table_html .= "<td>" . stripslashes($content_html). "...</td>" ;
							$table_html .= "<td>" ;
							$table_html .= "<div class=\"btn-group\" >" ;
							$table_html .= "<a class=\"btn btn-sm btn-warning\" href=\"?table=arquitetos&action=edit&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-pencil\"></span></a> " ;
							$table_html .= "<a onclick=\"return confirm('Are you sure you want to delete ID#". $data["id"]. "')\" class=\"btn btn-sm btn-danger\" href=\"?table=arquitetos&action=delete&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-trash\"></span></a> " ;
							$table_html .= "</div>" ;
							$table_html .= "</td>" ;
							$table_html .= "</tr>" ;
							$table_html .= "\r\n" ;
			...
			case "add":
				// TODO: ---- add
				$tags_html .= "<ul class=\"nav nav-tabs\">" ;
				$tags_html .= "<li><a href=\"?table=arquitetos&action=list\">".@LANG_LIST."</a></li>" ;
				$tags_html .= "<li class=\"active\"><a href=\"?table=arquitetos&action=add\">".@LANG_ADD."</a></li>" ;
				$tags_html .= "</ul>" ;
				$tags_html .= "<br/>" ;
				...
				if(isset($_POST["add"])){
					...
					/** get input **/
					if(isset($_POST["nome"])){
						$data_nome = addslashes($_POST["nome"]);
					}
					if(isset($_POST["foto"])){
				...
				if(isset($_FILES["foto-upload"]["name"])){
					...
					if(isset($_POST["conteudo"])){
						$data_conteudo = addslashes($_POST["conteudo"]);
					}
	
					/** prepare save to mysql **/
					$sql_query = "INSERT INTO `arquiteto` (`nome`,`foto`,`conteudo`) VALUES ('$data_nome','$data_foto','$data_conteudo')" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=arquitetos&action=list");
				}
	
	
				/** Create Form **/
				$tags_html .= '
				<form id="arquiteto" action="" method="post" enctype="multipart/form-data">
...
<div class="form-group">
	<label for="nome">' . @LANG_NOME. '</label>
	<input  id="nome" class="form-control input-nome" data-type="heading-1" type="text" name="nome" placeholder="" />
...
<div class="form-group">
	...
	<textarea id="foto" class="form-control input-foto"  data-type="images" name="foto" ></textarea>
	<p class="help-block">Using base64 image data (small images) <input data-target="#foto" type="file" data-type="image-base64" /> or file upload <input name="foto-upload" type="file" data-type="image-upload" /></p>
</div>
...
-->
<div class="form-group">
	<label for="conteudo">' . @LANG_CONTEUDO. '</label>
	<textarea id="conteudo" class="form-control input-conteudo"  data-type="to_trusted" name="conteudo" ></textarea>
	<p class="help-block"></p>
</div>
					<div class="form-group">
						<label for="add"></label>
		...
		case "edit":
			...
			if(isset($_GET["id"])){
				...
				if(isset($_FILES["foto-upload"]["name"])){
					...
					/** update data to sql **/
					$sql_query = "UPDATE `arquiteto` SET `nome` = '$data_nome',`foto` = '$data_foto',`conteudo` = '$data_conteudo' WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=arquitetos&action=list");
			...
			/** fetch current data **/
			$sql_query = "SELECT * FROM `arquiteto`  WHERE `id`=$entry_id LIMIT 0,1" ;
			if($result = $mysql->query($sql_query)){
				while ($data = $result->fetch_array()){
					$rows[] = $data;
				}
			...
			if(isset($rows[0]["nome"])){
				$data_nome = stripslashes($rows[0]["nome"]) ;
			}
			if(isset($rows[0]["foto"])){
				$data_foto = stripslashes($rows[0]["foto"]) ;
			}
			if(isset($rows[0]["conteudo"])){
				$data_conteudo = stripslashes($rows[0]["conteudo"]) ;
			}
	
			/** buat form edit **/
			$tags_html .= '
<form action="" method="post" enctype="multipart/form-data" id="arquiteto">
...
<div class="form-group">
	<label for="nome">' . @LANG_NOME. '</label>
	<input  id="nome" class="form-control input-nome" data-type="heading-1" type="text" name="nome" placeholder="" value="'.htmlentities($data_nome).'" />
	<p class="help-block"></p>
</div>
...
<div class="form-group">
	<label for="foto">' . @LANG_FOTO. '</label>
	<textarea id="foto" class="form-control input-foto"  data-type="images" name="foto" >'.htmlentities($data_foto).'</textarea>
	<p class="help-block">Using base64 image data (small images) <input data-target="#foto" type="file" data-type="image-base64" /> or file upload <input name="foto-upload" type="file" data-type="image-upload" /></p>
</div>
...
<div class="form-group">
	<label for="conteudo">' . @LANG_CONTEUDO. '</label>
	<textarea id="conteudo" class="form-control input-conteudo"  data-type="to_trusted" name="conteudo" >'.htmlentities($data_conteudo).'</textarea>
	<p class="help-block"></p>
</div>
<div class="form-group">
	...
	<input class="btn btn-primary" type="submit" name="edit" />
			...
			// TODO: ---- delete
				...
				if(isset($_GET["id"])){
					...
					$entry_id = (int)$_GET["id"];
					/** delete item in sql **/
					$sql_query = "DELETE FROM `arquiteto` WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=arquitetos&action=list");
		...
		$tags_html .= "<div class=\"container-fluid\">" ;
		$tags_html .= "<div class=\"row\">" ;
		$tags_html .= "<div class=\"col-md-12\"><p class=\"text-left navbar-text\">Copyright &copy; DigitalDesigners 2018 - All Rights Reserved</p></div>" ;
		$tags_html .= "</div>" ;
